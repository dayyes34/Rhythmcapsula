...
// Хранилище для отслеживания времени последнего действия пользователей
const userLastAction = {};
// Интервал между кликами в миллисекундах (например, 2 секунды)
const CLICK_TIMEOUT = 2000;

// Функция для проверки таймаута
function checkTimeout(userId) {
  const now = Date.now();
  if (userLastAction[userId] && (now - userLastAction[userId] < CLICK_TIMEOUT)) {
    return false; // Слишком рано для нового действия
  }
...
// Обработчик кнопки "Быстрая бронь" с защитой от частых кликов
bot.hears("⚡ Быстрая бронь", async (ctx) => {
  const userId = ctx.from.id;

  // Проверяем, не слишком ли часто кликает пользователь
  if (!checkTimeout(userId)) {
    return; // Игнорируем клик, если прошло мало времени
  }
...
// Защита от частых кликов для инлайн-кнопок
bot.on('callback_query', async (ctx) => {
  ...
  // Проверяем, не слишком ли часто кликает пользователь
  if (!checkTimeout(userId)) {
    // Отвечаем на callback_query, чтобы убрать анимацию загрузки кнопки
